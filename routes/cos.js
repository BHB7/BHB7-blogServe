...
// 检查必需的环境变量
if (!process.env.SecretId || !process.env.SecretKey || !process.env.Bucket || !process.env.Region) {
...
router.get('/', function (req, res, next) {
    ...
    if (!filename) {
        return res.status(400).send({ success: false, error: '请传入文件名' });
    }
    ...
    if (permission.limitExt) {
        ...
        if (extInvalid) {
            return res.status(400).send({ success: false, error: '非法文件，禁止上传' });
        }
    }
    ...
    getSts({ cosKey, condition })
        ...
        .catch((err) => {
            console.error('STS 错误', err);
            res.status(500).send({ success: false, error: '获取临时密钥失败', details: err });
        });
});
